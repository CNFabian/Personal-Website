...
const About = () => {
  ...
  const imageData: Record<string, { title: string; description: string; category: string; icon: string }> = {
    "Me and My Cat": {
      title: "My Cat & Me",
  ...
  const handleImageMouseEnter = (event: React.MouseEvent<HTMLImageElement>, alt: string) => {
    const target = event.currentTarget;
    const rect = target.getBoundingClientRect();

    // Check if image is on left or right side of page
    const isLeftSide = target.className.includes("left");
    const isRightSide = target.className.includes("right");
